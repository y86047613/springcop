...
<div class="row">
	<div class="col-xs-12" id="chongzhi_btn">
		<div class="well well-sm">
			<a>充值账户[邮箱]</a><input type="text" id="email" name="email">
			<a>充值房卡</a><input type="text" id="score" name="score">
			<a class="btn btn-info btn-sm" id="chongzhi">
				充值
			</a>
		</div>
	</div>
	<div class="col-xs-12" style="display: none" id="yinchang_btn">
		<div class="well well-sm">
			<a class="btn btn-info btn-sm" id="yinchang">
				继续充值
			</a>
		</div>
	</div>
...
<script type="text/javascript">
		var scripts = [ null, "${contextPath}/static/assets/js/date-time/bootstrap-datepicker.js","${contextPath}/static/assets/js/jquery.gritter.js", "${contextPath}/static/assets/js/date-time/locales/bootstrap-datepicker.zh-CN.js", "${contextPath}/static/assets/js/jqGrid/jquery.jqGrid.js", "${contextPath}/static/assets/js/jqGrid/i18n/grid.locale-cn.js", null ]
        $('.page-content-area').ace_ajax('loadScripts', scripts, function() {
        	...
        	jQuery(function($) {
        		...
        		jQuery(grid_selector).jqGrid({
        			...
        			datatype : "json",
        			height : "50%",
        			colNames : ['', 'ID', '姓名','金币', '性别', '邮箱', '联系电话', '生日', '所属部门', '角色', '是否禁用', '最后登录时间'],
        			colModel : [ {
        				name : '',
        				index : '',
        				width : 80,
		...
		$('#yinchang').on('click', function() {
			$('#chongzhi_btn').css("display","block");
			$('#yinchang_btn').css("display","none");
		})
		...
		$('#chongzhi').on('click', function() {
			...
			if($('#email').val() == ''){
				...
				$.ajax({
					dataType : "json",
					url : "${contextPath}" + "/sys/sysuser/chongzhi",
					type : "post",
					data : {
						email : $('#email').val(),
						score  :$('#score').val()
					...
					complete : function(data) {
						...
						if(success =='0'){
							...
							$('#chongzhi_btn').css("display","none");
							$('#yinchang_btn').css("display","block"); 
						}else{
							$.gritter.add({
				                title: '系统信息',
				                text: success,
